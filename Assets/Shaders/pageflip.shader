...
uniform vec2 rect;
uniform sampler2D previous_page;
uniform sampler2D current_page;

void fragment() {
	if (time >= flip_duration) {
		COLOR = texture(current_page, UV);
	} else {
		vec2 pt = UV * rect;
		...
		if (!onCylinder) {
			if (neg) {
				//color = vec4(1,1,0,1);
				// color = mix(0.1, 1.0, da) * texture(current_page, UV);		// next page
				color =  vec4(0,0,0,0);
			} else {
				//color = vec4(1,0,0,1);
				color = texture(previous_page, UV);						// previous page
			}
		} else {
			...
			if (next) {
				...
				color = texture(previous_page, (outside ? UV : p / rect));	// front and curved front
			}
			...
			if (!outside) {
				//color = vec4(0,1,0,1);
				color = texture(previous_page, p / rect);				// curved back
			}
		}
		COLOR = color;
	}
